...
class _EditProductPageState extends State<EditProductPage> {
  ...
  // ذخیره جستجوهای قبلی و نتایج آنها
  Map<String, bool> _searchHistory = {};

  // ذخیره مقادیر اصلی محصول بعد از یافتن
  String _originalId = '';
              ...
              Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Update Button - Smart Enable/Disable
                    SizedBox(
                      width: double.infinity,
                      child: ElevatedButton.icon(
                        onPressed: _hasChanges ? _updateProduct : null,
  ...
  void _searchProduct() {
    if (_searchFormKey.currentState!.validate()) {
      ...
      // چک کردن اینکه آیا این جستجو قبلا انجام شده
      if (_searchHistory.containsKey(searchQuery)) {
        // استفاده از نتیجه قبلی
        bool found = _searchHistory[searchQuery]!;
        ...
        // جستجوی جدید - تصمیم گیری رندوم
        final random = Random();
        final found = random.nextBool();

        // ذخیره نتیجه برای این جستجو
        _searchHistory[searchQuery] = found;
  ...
  void _fillProductData() {
    ...
    // ذخیره مقادیر اصلی
    _originalId = _idController.text;
    _originalTitle = _titleController.text;
    _originalPrice = _priceController.text;
    _originalDescription = _descriptionController.text;
    _originalImage = _imageController.text;
    _originalCategory = _categoryController.text;
    _originalRating = _ratingController.text;

    // ریست کردن flag تغییرات
    setState(() {
      _hasChanges = false;
    });
  }
  ...
  void _updateProduct() {
    if (_editFormKey.currentState!.validate()) {
      // آپدیت مقادیر اصلی با مقادیر جدید
      _originalId = _idController.text;
      _originalTitle = _titleController.text;
      _originalPrice = _priceController.text;
      _originalDescription = _descriptionController.text;
...
class _EditUserPageState extends State<EditUserPage> {
  ...
  // ذخیره مقادیر اصلی کاربر بعد از یافتن
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildTextField(
                      ...
                      icon: Icons.lock_outline,
                      validator: (value) {
                        // فقط اگر password تغییر کرده بود، confirm هم باید پر باشه
